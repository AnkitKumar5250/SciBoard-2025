...
/** A {@link ResizingBorderPane} that resizes the specified {@link Stage stage}. */
public class StageResizer extends BorderPane {
     ...
     * @param rootPane The root pane(set to be the center of this {@link BorderPane}).
     */
    public StageResizer(Stage stage, Pane rootPane) {
        windowWidth.bind(stage.widthProperty());
        windowHeight.bind(stage.heightProperty());

        windowMinimumWidth.bind(stage.minWidthProperty());
        windowMinimumHeight.bind(stage.minHeightProperty());
        ...
        this.setOnMousePressed(mouse -> {
            ...
            windowResizeStartedWidth.setValue(stage.getWidth());
            windowResizeStartedHeight.setValue(stage.getHeight());

            windowResizeStartedXPosition.setValue(stage.getX());
            windowResizeStartedYPosition.setValue(stage.getY());
        });
        ...
        this.configureBottomPane(stage);

        leftCenter = new Pane();
        rightCenter = new Pane();

        this.configureSidePanes(stage);
     ...
     * @param stage The stage to use for property bindings.
     */
    @SuppressWarnings("unused")
    private void configureTopPane(Stage stage) {
        this.topPane.getChildren().addAll(this.topLeftCorner,this.topCenter,this.topRightCorner);
        this.topPane.setSpacing(0);

        this.topPane.prefWidthProperty().bind(stage.widthProperty());
        ...
        this.configureHorizontalCenterPane(mouse -> {
            if (windowResizeStartedHeight.subtract(mouseResizeYTranslation).greaterThan(windowMinimumHeight).get()) {
                stage.setY(windowResizeStartedYPosition.add(mouseResizeYTranslation).get());
                stage.setHeight(windowResizeStartedHeight.subtract(mouseResizeYTranslation).get());
            }
        ...
        this.configureCornerPane(mouse -> {
            if (windowResizeStartedWidth.subtract(mouseResizeXTranslation).greaterThan(windowMinimumWidth).get()) {
                stage.setX(windowResizeStartedXPosition.add(mouseResizeXTranslation).get());
                stage.setWidth(windowResizeStartedWidth.subtract(mouseResizeXTranslation).get());
            }
        ...
        this.configureCornerPane(mouse -> {
            if (windowResizeStartedWidth.add(mouseResizeXTranslation).greaterThan(windowMinimumWidth).get()) {
                stage.setWidth(windowResizeStartedWidth.add(mouseResizeXTranslation).get());
            }
     ...
     */
    @SuppressWarnings("unused")
    private void configureBottomPane(Stage stage) {
        ...
        this.bottomPane.prefWidthProperty().bind(stage.widthProperty());
        ...
        this.configureHorizontalCenterPane(mouse -> {
            if (windowResizeStartedHeight.add(mouseResizeYTranslation).greaterThan(windowMinimumHeight).get()) {
                stage.setHeight(windowResizeStartedHeight.add(mouseResizeYTranslation).get());
            }
     ...
     */
    @SuppressWarnings("unused")
    private void configureSidePanes(Stage stage) {
        this.configureSidePane(mouse -> {
            if (windowResizeStartedWidth.subtract(mouseResizeXTranslation).greaterThan(windowMinimumWidth).get()) {
                stage.setX(windowResizeStartedXPosition.add(mouseResizeXTranslation).get());
                stage.setWidth(windowResizeStartedWidth.subtract(mouseResizeXTranslation).get());
            }
        }, leftCenter, stage, Cursor.W_RESIZE);

        this.configureSidePane(mouse -> {
            if (windowResizeStartedWidth.add(mouseResizeXTranslation).greaterThan(windowMinimumWidth).get()) {
                stage.setWidth(windowResizeStartedWidth.add(mouseResizeXTranslation).get());
            }
        }, rightCenter, stage, Cursor.E_RESIZE);
     ...
     * @param onMouseDragged The resize function.
     * @param centerPane The pane to configure.
     * @param stage The stage to bind properties to.
     * @param cursor The cursor to display on hover.
     * @return The configured pane.
     */
    private Pane configureSidePane(EventHandler<? super MouseEvent> onMouseDragged, Pane centerPane, Stage stage, Cursor cursor) {
        centerPane.prefWidthProperty().bind(this.paneThickness);
        centerPane.prefHeightProperty().bind(stage.heightProperty().multiply(this.borderLength));
